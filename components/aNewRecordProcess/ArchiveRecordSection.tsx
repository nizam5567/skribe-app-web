...
import SmallDownloadIcon from '../v2/svg-components/SmallDownloadIcon';
import ArchiveWrittenRecord from '../v2/the-record/ArchiveWrittenRecord';
import VideoRecord from '../v2/the-record/VideoRecord';
import WrittenRecord from '../v2/the-record/WrittenRecord';
import RecordProcessSteps from './RecordProcessSteps';
...
const ArchiveRecordSection = ({ eventId }: TheRecord) => {
  ...
  useEffect(() => {
    const intervalTranscriptStatus = setInterval(async () => {
      if (eventId && transcriptStatus !== 'PDF_COMPLETE') {
        await getTranscriptStatus(eventId);
      }
    }, 20000);

    if (transcriptStatus === 'PDF_COMPLETE') {
      getVideoInformation(eventId);
  ...
  return (
    <>
      {!transcriptStatus || (transcriptStatus !== 'TRANSCRIPT_ASR_COMPLETE' && !transcriptStatus.includes('PDF') && transcriptStatus !== 'TRANSCRIPTION_ASR_COMPLETE')
        ? (
...
export default ArchiveRecordSection;

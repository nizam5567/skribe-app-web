...
const StipulationSection = ({ partyDataFound }: IStipulationSection) => {
  ...
  return (
    <Box borderRadius={2} px={3} py={4.5} mb={1.5} sx={{ 'background': '#fff', 'border': '1px solid #E8EBF2', 'boxShadow': '-4px 8px 24px rgba(44, 63, 88, 0.02)' }}>
      ...
      <Box display="flex" justifyContent={'space-between'} alignItems="flex-start" sx={{ 'flexDirection': { 'xs': 'column-reverse', 'md': 'row' }, 'borderBottom': '1px solid #e6e6e6' }} pb={2}>
        <Box>
          ...
          <Typography sx={{ 'fontSize': '16px', 'fontWeight': 400, 'lineHeight': '21px', 'color': '#3F434A' }} mt={1.5}>
            Send a stipulation to other parties to cover this event&apos;s admissibility or choose not to use one
          </Typography>
        </Box>
        {userTenantId && event?.tenantid === userTenantId && event.status !== 'IN_PROGRESS' && event.status !== 'LIVE' && event.status !== 'COMPLETE' && (
          <Box display="flex" justifyContent="flex-end">
            <SecondaryButton text="Edit" onClickFn={handleStipulation} />
          </Box>
        )}
      </Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mt={3} ml={3}>
        ...
        {partyDataFound && parties && parties.length >= 2 // <PartyStipulationStatus />
          ? null : !partyDataFound
            ? (

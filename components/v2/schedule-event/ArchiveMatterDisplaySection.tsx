...
const ArchiveMatterDisplaySection = () => {
    ...
    <Box>
      <Box mt={2} display="flex" alignItems="flex-start" justifyContent="space-between" sx={{ 'width': '100%' }}>
        ...
        {/* {event && event.status !== "COMPLETE" && (
          <Box display="flex" flexDirection="column" alignItems="flex-end" sx={{ marginTop: "46px" }}>
            {event?.datestart && (
              <Fragment>
                <Typography sx={{ fontWeight: 400, fontSize: "14px", lineHeight: "17px" }} color="#000">
                  Event starts at
                </Typography>
                <Box mt={1}>
                  {" "}
            ...
            {readyToShowEventStartJoinButton && (
              <Box mt={2}>
                <Button
                  variant="contained"
                  disableElevation
                  ...
                  sx={{ fontSize: "14px", lineHeight: "17px", fontWeight: 400, background: "#02178C", padding: "10px 20px", color: "#fff" }}
                >
                  {userTenantId && userTenantId !== event?.tenantid ? "Join Event" : "Start Event"}
                </Button>
              </Box>
            )}
          </Box>
...
export default ArchiveMatterDisplaySection;

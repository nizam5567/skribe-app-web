...
import { Box, TextField, Button, CircularProgress, Typography, FormHelperText, FormControl, InputLabel, Select, MenuItem, FormControlLabel, Checkbox } from '@mui/material';
...
import { EventStatus, UpdateEventRequestEventtypeEnum } from '../../../openapi';

const CreateEventWithName = () => {
  const { accessToken } = useAuthContext();
  const dispatch = useDispatch();
  const { matter } = useAppSelector((state: RootState) => state.mattersReducer);
  const alert = boundSnackbarActions;
  const router = useRouter();
  const [isEventArchive, setIsEventArchive] = useState(false);
  ...
  const validationSchema = Yup.object({
    ...
    'isArchive': Yup.boolean().nullable()
  });
  ...
  const handleFormChange = (e: any) => {
    if (e.target.name === 'isArchive') {
      if (e.target.checked) {
        setIsEventArchive(true);
      } else {
        setIsEventArchive(false);
      }
    }
  };
            ...
            <Form onChange={handleFormChange}>
              <Box sx={{ 'display': 'flex', 'flexDirection': 'column', 'justifyContent': 'center', 'alignItems': 'flex-start' }}>
                ...
                <Box mt={2.5} display="flex" flexDirection={{ 'xs': 'column', 'md': 'row' }} justifyContent="space-between" sx={{ 'width': '100%' }}>
                  <FormControlLabel control={<Checkbox checked={formik.values.isArchive} />} label="This event was in the past" name="isArchive" onChange={formik.handleChange} />
                </Box>
                ...
                {!isEventArchive
                  ...
                  <>
                    ...
                    <Box mt={1} display="flex" flexDirection={{ 'xs': 'column', 'md': 'row' }} justifyContent="space-between" sx={{ 'width': '100%' }}>
...
export default CreateEventWithName;
